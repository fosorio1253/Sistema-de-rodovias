...
namespace DER.WebApp.Infra.DAO
{
    public class DominioPistaTiposDAO : BaseDAO<DominioPistaTipos>
    {
        ...
        {
            logger = new Logger("DominioPistaTipos", context);
        }
        ...
        public bool Update(DominioPistaTipos domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.ptp_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DominioPistaTipos model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_DOMINIOPISTATIPOS", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.ptp_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
