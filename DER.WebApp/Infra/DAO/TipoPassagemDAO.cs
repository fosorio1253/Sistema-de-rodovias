...
namespace DER.WebApp.Infra.DAO
{
    public class TipoPassagemDAO : BaseDAO<TipoPassagem>
    {
        ...
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var oldValue = GetById(domain.tipo_passagem_id);
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                ...
                var value = GetById(domain.tipo_passagem_id);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public TipoPassagem GetById(int id)
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var retorno = new TipoPassagem();
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_SEL_TIPO_PASSAGEM_ID", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
        ...
        {
            var oldValue = GetById(model.tipo_passagem_id);
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
