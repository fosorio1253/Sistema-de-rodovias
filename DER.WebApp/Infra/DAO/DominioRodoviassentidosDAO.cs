...
namespace DER.WebApp.Infra.DAO
{
    public class DominioRodoviassentidosDAO : BaseDAO<DominioRodoviassentidos>
    {
        ...
        {
            logger = new Logger("DominioRodoviassentidos", context);
        }
        ...
        public bool Update(DominioRodoviassentidos domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.sen_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DominioRodoviassentidos model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_DOMINIORODOVIASSENTIDOS", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.sen_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
