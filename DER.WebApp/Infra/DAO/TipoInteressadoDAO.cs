...
namespace DER.WebApp.Infra.DAO
{
    public class TipoInteressadoDAO : BaseDAO<TipoInteressado>
    {
        ...
        {
            logger = new Logger("TipoInteressado", context);
        }
        ...
        public List<TipoInteressado> ObtemTodos()
        {
            ...
            try
            {
                ...
                {
                    using (var command = new SqlCommand("STP_SEL_TIPOINTERESSADO", conn))
                    {
        ...
        public bool Inserir(TipoInteressado domain)
        {
            ...
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_INS_TIPOINTERESSADO", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
        ...
        public bool Update(TipoInteressado domain)
        {
            ...
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_UPD_TIPOINTERESSADO", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
                        command.Parameters.Add(new SqlParameter("@tipo_interessado_id", domain.tipo_interessado_id));
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.tipo_interessado_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public TipoInteressado GetById(int id)
        {
            ...
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_SEL_TIPOINTERESSADO_ID", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
        ...
        public bool Delete(TipoInteressado model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_TIPOINTERESSADO", conn))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.tipo_interessado_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
