...
namespace DER.WebApp.Infra.DAO
{
    public class DominioPorticosPmvsTiposDAO : BaseDAO<DominioPorticosPmvsTipos>
    {
        ...
        {
            logger = new Logger("DominioPorticosPmvsTipos", context);
        }
        ...
        public bool Update(DominioPorticosPmvsTipos domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.ppt_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DominioPorticosPmvsTipos model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_DOMINIOPORTICOSPMVSTIPOS", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.ppt_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
