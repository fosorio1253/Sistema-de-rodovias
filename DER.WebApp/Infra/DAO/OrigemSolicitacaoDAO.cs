...
namespace DER.WebApp.Infra.DAO
{
    public class OrigemSolicitacaoDAO : BaseDAO<OrigemSolicitacao>
    {
        ...
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var oldValue = GetById(domain.origem_da_solicitacao_id);
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                ...
                var value = GetById(domain.origem_da_solicitacao_id);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public OrigemSolicitacao GetById(int id)
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var retorno = new OrigemSolicitacao();
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_SEL_ORIGEM_SOLICITACAO_ID", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
        ...
        {
            var oldValue = GetById(model.origem_da_solicitacao_id);
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
