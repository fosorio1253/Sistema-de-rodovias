...
namespace DER.WebApp.Infra.DAO
{
    public class TipoOcupacaoDAO : BaseDAO<TipoOcupacao>
    {
        ...
        {
            logger = new Logger("TipoOcupacao", context);
        }
        ...
        public bool Update(TipoOcupacao domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.tipo_ocupacao_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(TipoOcupacao model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_TIPOOCUPACAO", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.tipo_ocupacao_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
