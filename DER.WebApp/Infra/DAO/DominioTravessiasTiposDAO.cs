...
namespace DER.WebApp.Infra.DAO
{
    public class DominioTravessiasTiposDAO : BaseDAO<DominioTravessiasTipos>
    {
        ...
        {
            logger = new Logger("DominioTravessiasTipos", context);
        }
        ...
        public bool Update(DominioTravessiasTipos domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.trt_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DominioTravessiasTipos model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_DOMINIOTRAVESSIASTIPOS", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.trt_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
