...
namespace DER.WebApp.Infra.DAO
{
    public class SituacaoOcupacaoDAO : BaseDAO<SituacaoOcupacao>
    {
        ...
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var oldValue = GetById(domain.situacao_ocupacao_id);
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                ...
                var value = GetById(domain.situacao_ocupacao_id);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public SituacaoOcupacao GetById(int id)
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var retorno = new SituacaoOcupacao();
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_SEL_SITUACAO_OCUPACAO_ID", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
        ...
        {
            var oldValue = GetById(model.situacao_ocupacao_id);
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
