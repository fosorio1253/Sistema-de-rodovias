...
namespace DER.WebApp.Infra.DAO
{
    public class DominioSubestacoesReservatoriosTiposDAO : BaseDAO<DominioSubestacoesReservatoriosTipos>
    {
        ...
        {
            logger = new Logger("DominioSubestacoesReservatoriosTipos", context);
        }
        ...
        public bool Update(DominioSubestacoesReservatoriosTipos domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.srt_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DominioSubestacoesReservatoriosTipos model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_DOMINIOSUBESTACOESRESERVATORIOSTIPOS", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.srt_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
