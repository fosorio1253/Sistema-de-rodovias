...
namespace DER.WebApp.Infra.DAO
{
    public class TipoDocumentoDAO : BaseDAO<TipoDocumentos>
    {
        ...
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var oldValue = GetById(domain.tipo_documento_id);
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                ...
                var value = GetById(domain.tipo_documento_id);
                return true;
            }
            catch (Exception ex)
            {
        ...
        public TipoDocumentos GetById(int id)
        {
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
            var retorno = new TipoDocumentos();
            try
            {
                using (var conn = new SqlConnection(connectionString))
                {
                    using (var command = new SqlCommand("STP_SEL_TIPO_DOCUMENTOS_ID", conn))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        conn.Open();
        ...
        {
            var oldValue = GetById(model.tipo_documento_id);
            var connectionString = ConfigurationManager.ConnectionStrings["DerContext"].ConnectionString;
