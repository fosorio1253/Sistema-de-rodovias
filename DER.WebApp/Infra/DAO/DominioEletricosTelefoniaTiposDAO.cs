...
namespace DER.WebApp.Infra.DAO
{
    public class DominioEletricosTelefoniaTiposDAO : BaseDAO<DominioEletricosTelefoniaTipos>
    {
        ...
        {
            logger = new Logger("DominioEletricosTelefoniaTipos", context);
        }
        ...
        public bool Update(DominioEletricosTelefoniaTipos domain)
        {
            ...
            try
            {
                ...
                if (!value.Equals(oldValue))
                    logger.salvarLog(TipoAlteracao.Edicao, domain.ett_id.ToString(), logger.serializer.Serialize(oldValue), logger.serializer.Serialize(value));
                return true;
            }
            catch (Exception ex)
            {
        ...
        public bool Delete(DominioEletricosTelefoniaTipos model)
        {
            ...
            using (var conn = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand("STP_DEL_DOMINIOELETRICOSTELEFONIATIPOS", conn))
                {
                    ...
                    command.Parameters.Clear();
                    conn.Close();
                    logger.salvarLog(TipoAlteracao.Exclusao, model.ett_id.ToString(), logger.serializer.Serialize(oldValue), "");
                }
            }
            return true;
        }
